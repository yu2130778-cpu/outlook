...
def _extract_refresh_token_device_code(browser, email, client_id="14d82eec-204b-4c2f-b7e8-296a70dab67e", timeout=120, password="", proxy_url=""):
    """使用 Device Code 流程获取 OAuth refresh_token（更可靠，不需要 localhost 回调）。
    流程：
    1. 请求 device_code + user_code
    2. 用 CDP 浏览器打开验证页，自动填写邮箱/密码/验证码
    3. 轮询 token 端点直到用户完成授权
    优势：不需要 localhost 回调服务器，避免代理拦截/端口占用问题。
    """
    import json as _json
    ...
    def _post_form(url, form_data, use_proxy=True):
        """发送 POST 表单请求，支持代理（使用 requests 替代 urllib，修复代理 HTTPS 超时）"""
        try:
    ...
    logger.info("[DC_RT] 请求 Device Code: email=%s, client_id=%s", email, client_id)
    dc_data, dc_err = _post_form(device_url, {"client_id": client_id, "scope": scopes})
    if dc_err or not dc_data:
        logger.warning("[DC_RT] 请求 Device Code 失败: %s", dc_err)
        return ""
    ...
    if not user_code or not device_code:
        logger.warning("[DC_RT] 微软未返回 device_code/user_code")
        return ""

    logger.info("[DC_RT] Device Code 获取成功: user_code=%s, verification_uri=%s", user_code, verification_uri)

    # Step 2: 用 CDP 浏览器打开验证页并自动填写
    target_url = verification_uri_complete if verification_uri_complete else verification_uri
    logger.info("[DC_RT] 导航到验证页: %s", target_url)
    ...
    def _poll_worker():
        ...
        while time.time() < deadline:
            ...
            if resp and resp.get("refresh_token"):
                ...
                logger.info("[DC_RT] 轮询获取到 tokens!")
                return
            if resp and resp.get("access_token"):
                # 有时微软只返回 access_token 没有 refresh_token
                with _poll_lock:
                    _poll_result["tokens"] = resp
                    _poll_result["done"] = True
                logger.info("[DC_RT] 轮询获取到 access_token (无 refresh_token)")
                return
            if err:
                ...
                if err_code == "authorization_pending":
                    ...
                    interval += 5
                    logger.info("[DC_RT] 微软要求放慢轮询，间隔 %d 秒", interval)
                elif err_code == "expired_token":
                    logger.warning("[DC_RT] Device Code 已过期")
                    with _poll_lock:
                        _poll_result["error"] = "expired"
                        _poll_result["done"] = True
                    return
                elif err_code == "authorization_declined":
                    logger.warning("[DC_RT] 用户拒绝了授权")
                    with _poll_lock:
                        _poll_result["error"] = "declined"
                        _poll_result["done"] = True
                    return
                else:
                    logger.warning("[DC_RT] 轮询错误: %s - %s", err_code, err.get("error_description", ""))
        logger.warning("[DC_RT] 轮询超时 (%d 秒)", expires_in)
        with _poll_lock:
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            logger.info("[DC_RT] 页面: URL=%s body_len=%d", url[:120], len(body))

            # ── 检测验证码输入页（Device Code 验证页） ──
            if "microsoft.com/link" in url_lower or "devicelogin" in url_lower or "oauth20_remoteconnect" in url_lower:
                ...
                if has_code_input and user_code:
                    logger.info("[DC_RT] 检测到验证码输入框，填入 user_code: %s", user_code)
                    ...
                    logger.info("[DC_RT] 验证码填写结果: %s", fill_result)
                    time.sleep(1)
                    # 点击 Next/Submit
                    browser.evaluate("""(() => {
                        const btns = document.querySelectorAll('button, input[type=submit], [role=button]');
                        for (const b of btns) {
                            const t = (b.textContent || b.value || '').toLowerCase().trim();
                            if (t.includes('next') || t.includes('submit') || t.includes('continue') || t.includes('下一步') || t.includes('继续')) {
            ...
            if (url_host in ("login.live.com", "login.microsoftonline.com")) and "error" not in body:
                if any(kw in body for kw in ["pick an account", "choose account", "选择帐户", "选择账户", "sign in with"]):
            ...
            if is_account_picker and email:
                logger.info("[DC_RT] 账号选择页，点击已登录账号: %s", email)
                ...
                if pos:
                    try:
                        ...
                        logger.info("[DC_RT] 找到账号: '%s' at (%d,%d)", pos_data.get('text',''), pos_data['x'], pos_data['y'])
                        browser.click_at(pos_data['x'], pos_data['y'])
                    except Exception as e:
                        logger.warning("[DC_RT] 账号点击失败: %s", e)
                else:
            ...
            # ── 恢复信息页（proofs/Add） ──
            if "proofs" in url_lower and "add" in url_lower:
                logger.info("[DC_RT] 恢复信息页，尝试跳过...")
                skip_btn = browser.evaluate("""(() => {
                    const btns = document.querySelectorAll('button, a, [role=button]');
                    for (const b of btns) { const t=(b.textContent||'').toLowerCase();
                        if (t.includes('skip')||t.includes('暂不')||t.includes('跳过')||t.includes('not now')||t.includes('cancel')||t.includes('取消'))
                        { b.click(); return t.substring(0,30); } }
                    return null;
                })()""")
                if not skip_btn:
                    logger.warning("[DC_RT] 恢复页无法跳过，立即返回")
                    return ""
                time.sleep(3)
                continue

            # ── 邮箱输入页（Device Code 授权后的登录第一步） ──
            if (url_host in ("login.live.com", "login.microsoftonline.com", "login.live-int.com")) and email:
                ...
                if has_email_field and not has_pwd_field:
                    logger.info("[DC_RT] 邮箱输入页，填入邮箱: %s", email)
                    fill_result = browser.evaluate(f"""(() => {{
                        const inputs = document.querySelectorAll('input[name="loginfmt"], input[type="email"], #i0116');
                        for (const inp of inputs) {{
                            if (inp.offsetParent !== null && inp.offsetWidth > 30) {{
                    ...
                    logger.info("[DC_RT] 邮箱填写结果: %s", fill_result)
                    time.sleep(3)
                    continue
            ...
            if (url_host in ("login.live.com", "login.microsoftonline.com")) and password:
                ...
                if has_password_field:
                    ...
                    fill_result = browser.evaluate(f"""(() => {{
                        ...
                        for (const inp of inputs) {{
                            if (inp.offsetParent !== null && inp.offsetWidth > 30) {{
                                ...
                                inp.value = '';
                                const nativeSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
                                nativeSetter.call(inp, '{password.replace("'", "\\'")}');
                                inp.dispatchEvent(new Event('input', {{bubbles: true}}));
                                inp.dispatchEvent(new Event('change', {{bubbles: true}}));
                                // 点击 Sign in / Next 按钮
                                const btns = document.querySelectorAll('button, input[type=submit], [role=button]');
                    ...
                    logger.info("[DC_RT] 密码填写结果: %s", fill_result)
            ...
            # ── 通行密钥弹窗 ──
            if any(kw in body for kw in ["选择保存通行密钥", "windows 安全中心", "windows security", "passkey setup", "passkey creation"]):
                logger.info("[DC_RT] 通行密钥弹窗，点击跳过...")
                browser.evaluate("""(() => {
                    const btns = document.querySelectorAll('button, [role=button], input[type=submit]');
                    for (const b of btns) {
                        const t = (b.textContent || b.value || '').toLowerCase().trim();
            ...
            is_consent = ("account.live.com/consent" in url_lower or
                         ...
                         any(kw in body for kw in ["permission", "permissions requested", "请求的权限", "已授予访问权限"]))
            if is_consent:
                logger.info("[DC_RT] Consent 页面，点击确认...")
                browser.evaluate("""(() => {
                    ...
                    for (const b of btns) {
                        ...
                        if (t.includes('accept')||t.includes('agree')||t.includes('allow')||t.includes('continue')||
                            ...
                            t.includes('同意')||t.includes('接受')||t.includes('继续')||t.includes('下一步'))
            ...
            # ── 隐私声明页 ──
            if any(kw in body for kw in ["privacynotice", "privacy notice", "隐私声明", "隐私偏好"]):
                logger.info("[DC_RT] 隐私声明页")
                browser.evaluate("""(() => {
                    const btns = document.querySelectorAll('button, input[type=submit], [role=button]');
                    for (const b of btns) {
                        const t = (b.textContent||b.value||'').toLowerCase();
                        if (t.includes('agree')||t.includes('continue')||t.includes('同意')||t.includes('接受')||t.includes('继续'))
                        { b.click(); return t.substring(0,30); }
                    }
                    return null;
                })()""")
            ...
            if "fido" in url or "passkey" in body or "security key" in body or "windows hello" in body:
                logger.info("[DC_RT] Passkey 页，跳过")
                browser.evaluate("""(() => {
                    const btns = document.querySelectorAll('button, a, [role=button]');
                    for (const b of btns) { const t=(b.textContent||'').toLowerCase();
                        if (t.includes('skip')||t.includes('not now')||t.includes('暂不')||t.includes('以后')||t.includes('稍后')||t.includes('no thanks'))
                        { b.click(); return t.substring(0,30); } }
                    return null;
                })()""")
                time.sleep(3)
            ...
            if "chromewebdata" in url_lower:
                logger.warning("[DC_RT] chrome-error 页面，3s 后重新导航")
                time.sleep(3)
                try:
        ...
        with _poll_lock:
            if _poll_result["done"] and _poll_result["tokens"]:
                ...
                if rt:
                    logger.info("[DC_RT] ✅ refresh_token 获取成功: %s...", rt[:30])
                else:
                    logger.warning("[DC_RT] token 响应中无 refresh_token")
                return rt
            elif _poll_result["error"]:
                logger.warning("[DC_RT] Device Code 流程失败: %s", _poll_result["error"])
                return ""
            else:
                logger.warning("[DC_RT] 等待超时")
                return ""

    except Exception as e:
        logger.warning("[DC_RT] Device Code 流程异常: %s", e)
        return ""
    finally:
        with _poll_lock:
            _poll_result["done"] = True  # 通知轮询线程退出


def _extract_refresh_token(browser, email, client_id="14d82eec-204b-4c2f-b7e8-296a70dab67e", timeout=60, password="", proxy_url=""):
    """使用 localhost 回调 + PKCE 获取 OAuth refresh_token（备用方法）。
            ...
            if page_state == "account_home":
                ...
                if extract_rt and browser:
                    ...
                    try:
                        ...
                        rt = _extract_refresh_token_device_code(browser, account.email, account.client_id or "14d82eec-204b-4c2f-b7e8-296a70dab67e", password=account.password, proxy_url=_rt_proxy)
                        if not rt:
                            logger.info("[CDP_REG] Device Code 流程未获取到 RT，尝试 Authorization Code 流程...")
