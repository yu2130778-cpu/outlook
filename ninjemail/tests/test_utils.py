...
class TestGenerateMissingInfo:
  ...
  def test_generated_password_has_high_entropy_shape(self):
    passwords = [generate_missing_info("user", "", "John", "Doe", "USA", "1980-01-01")[1] for _ in range(100)]
    assert len(set(passwords)) == len(passwords)
    for password in passwords:
      assert 18 <= len(password) <= 24
  ...
  def test_generated_usernames_have_secure_suffixes(self):
    usernames = [
      generate_missing_info("", "Password123!", "John", "Doe", "USA", "1980-01-01")[0]
      for _ in range(200)
    ]
