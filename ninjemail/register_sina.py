#!/usr/bin/env python3
"""
新浪邮箱 (sina.com) 独立注册脚本
用法: python register_sina.py [--headless] [--proxy http://ip:port]
"""
...
SIGNUP_URL = "https://mail.sina.com.cn/register/regmail.php"
...
def register(account: AccountInfo | None = None,
    ...
    try:
        ...
        # 邮箱地址
        for sel in ["#emailName", "input[name='email']", "input[id*='email' i]",
                     "input[placeholder*='邮箱']", "input[type='text']"]:
        ...
        for sel in ["input[name='phone']", "input[type='tel']",
            ...
            if nid and browser.is_element_visible(nid):
                ...
                logger.warning("[WARN] Sina 要求手机号验证")
        ...
        if captcha:
            ...
            take_screenshot(browser, "sina_captcha")
        ...
        result.error = str(e)
        logger.exception("[ERROR] Sina 注册异常")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sina 邮箱注册")
